    ...
    <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                    <div class="courses-item d-flex flex-column bg-white overflow-hidden h-100">
                        <div class="text-center p-4 pt-0">
                            <div class="d-inline-block bg-primary text-white fs-5 py-1 px-4 mb-4">Yangilik</div>
                            <h5 class="mb-3">${item.title}</h5>
                            <p>${item.description
}</p>
                            <ol class="breadcrumb justify-content-center mb-0">
                                <li class="breadcrumb-item small"><i class="fa fa-signal text-primary me-2"></i>${item.type == 'event'?'Tadbir':item.type == 'news'?'Yangilik':item.type == 'elon'?'Elon':''}</li>
                                <li class="breadcrumb-item small"><i class="fa fa-calendar-alt text-primary me-2"></i>${item.created_at
}</li>
<li class="breadcrumb-item small"><i class="fa fa-user text-primary me-2"></i>${item.users.first_name} ${item.users.last_name}</li>
                            </ol>
                        </div>
                        <div class="position-relative mt-auto">
                            <img class="img-fluid" src=${item.image_url} alt="">
                            <div class="courses-overlay">
                                <a class="btn btn-outline-primary border-2" href="more.html?id=${item.id}">Ko'proq Bilish</a>
                            </div>
                        </div>
                    </div>
                </div>
