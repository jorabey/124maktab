...
// Get User ["teacher", "manager", "library"]

async function getUsers() {
  const { data, error } = await sb
    .from("profiles")
    .select("*")
    .in("role", ["teacher", "manager", "library", "manager-teacher"]);
  ...
  data.forEach((user, i) => {
    ...
    usersContainer.innerHTML += `
    <tr style="cursor: pointer;" onclick=getSchoolUserInfo("${user.id}")>
                            <td>${i + 1}</td>
                            <td>
                              <img src="${
                                user.avatar_url ||
...
async function getSchoolUserInfo(userId) {
...
/*
  async function setInfo(userId) {
        const { data, error } = await sb
          .from("profiles")
          .select("*")
          .in("role", ["teacher", "manager", "library"]);
        ...
        data.forEach((user) => {
          if (user.id == userId) {
            const login = ($("login2").value = user.login);
            const password = ($("password2").value = user.password);
            const first = ($("first_name2").value = user.first_name);
            const last = ($("last_name2").value = user.last_name);
            const phone = ($("tel2").value = user.phone);
            const ord = ($("role2").value = user.gender);
            const position = ($("position2").value = user.position);
            const passport_serial = ($("passport_serial2").value =
              user.passport_serial);
            const midl = ($("middle_name2").value = user.middle_name);
            const gender = ($("gender2").value = user.gender);
            const birth = ($("birth_date2").value = user.birth_date); // YYYY-MM-DD yoki null
            localStorage.setItem("avatarUF", user.avatar_url);
            const email = ($("email2").value = user.email);
          }
        });
      }
