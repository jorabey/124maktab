...
async function getRax() {
  ...
  data.forEach((user,i)=>{
    ...
    <tr style="cursor: pointer;" onclick=getRaxInfo("${user.id || null}")>
                            <td>${i+1}</td>
                            <td>
                              <img src="${user.avatar_url || '../../assets/images/jpg/user-icon.jpg'}" class="me-2" alt="image">${[user.first_name||"", user.last_name||""].join(" ").trim() || user.login}
                            </td>
                            <td> ${user.position || '-'} </td>
                            <td>
                              <label class="badge ${isOnline ? 'badge-gradient-success' : 'badge-gradient-danger'}">${isOnline ? 'Onlayn' : 'Oflayn'}</label>
                            </td>
                            <td> ${user.email || '-'} </td>
                            <td> ${user.phone || '-'} </td>
 </tr>
...
data.forEach((user)=>{
  if (user.id == userId) {
    ...
    <img height="200" class="form-control" src="${user.avatar_url || '../../assets/images/jpg/user-icon.jpg'}">
  </div>
  ...
  <div class="col-md-4">
    ...
    <label class="badge form-control ${isOnline ? 'badge-gradient-success' : 'badge-gradient-danger'}">${isOnline ? 'Onlayn' : 'Oflayn'}</label>
  </div>
  
   <div class="col-md-4">
    <label class="form-label">Tug'ilgan Sana</label>
    <input type="text" class="form-control" placeholder=${user.birth_date || '-'}  disabled>
  </div>
  <div class="col-md-4">
  ...
  <div class="col-md-4">
    ...
    <input type="text" class="form-control"  placeholder=${user.passport_serial || '-'}  disabled>
  </div>
  <div class="col-md-4">
    <label class="form-label">Yashash Manzil</label>
    <input type="text" class="form-control"  placeholder=${user.location || '-'}  disabled>
  </div>
  <div class="col-md-4">
  ...
  <div class="col-md-4">
    ...
    <input type="text" class="form-control"  placeholder=${user.login || '-'}  disabled>
  </div>
  <div class="col-12">
    <button onclick=ed() type="button" class="btn btn-success">Tahrirlash</button>
    <button onclick=deleteUser("${user.id || null}") type="button" class="btn btn-danger">O'chirish</button>
  </div>
    `
  }
})
...
async function deleteUser(userId) {
  loader(true)
  try {
    const res = await fetch("https://maktab-backend-six.vercel.app/api/delete-user", {
      method: "POST",
      headers: {
...
$('addUserForm').addEventListener('submit', async (e)=>{
 ...
 const response = await fetch('https://maktab-backend-six.vercel.app/api/admin-signup', {
    method: 'POST',
    body: data
  });
...
loader(false)
result.success?showAlert("Hodim Qo'shildi",'success'):showAlert(result.error,'err')
  $('addUserForm').reset()
  closeInfo()
  startPresence()
